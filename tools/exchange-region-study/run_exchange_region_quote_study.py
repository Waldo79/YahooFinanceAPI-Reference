#!/usr/bin/env python3
r"""Run Study 03: an international exchange and region Quote baseline.

The study holds the endpoint, request parameters, session mode, and expected
Yahoo quote type constant while varying the primary listing exchange and
geographic market. Every subject is a reviewed ordinary-equity listing.
...
DEFAULT_CONFIG = REPOSITORY_ROOT / "config" / "studies" / "study-03-international-exchange-region.json"
...
REGION_SUMMARY_COLUMNS = [
    ...
    "countries_or_markets_json",
    "currencies_json",
    "exchange_labels_json",
    "symbols_json",
]
...
CURRENCY_SUMMARY_COLUMNS = [
    "expected_currency",
    "subject_count",
    "returned_currency_match_count",
    "returned_currencies_json",
    "symbols_json",
]
...
def load_study_definition(path: Path) -> tuple[dict[str, Any], EndpointDefinition, list[SubjectDefinition]]:
    ...
    if endpoint.endpoint_id != "quote":
        raise StudyError("Study 03 requires endpoint_id=quote.")
    if endpoint.method != "GET":
        raise StudyError("Study 03 currently supports GET only.")
    if "{symbol}" not in canonical_json(endpoint.params):
        raise StudyError("Study 03 endpoint params must contain a {symbol} placeholder.")
    ...
    for raw in raw_subjects:
        subject = SubjectDefinition(
            ...
            geographic_region=str(raw["geographic_region"]),
            country_or_market=str(raw["country_or_market"]),
        ...
        market_key = (subject.country_or_market, subject.expected_exchange_label)
        if market_key in seen_market_keys:
            raise StudyError(
                "Study 03 requires one primary listing per country/exchange panel entry; "
                f"duplicate market key: {market_key}"
            )
        if subject.expected_quote_type != "EQUITY":
            raise StudyError(
                f"Study 03 ordinary-equity control requires expected_quote_type=EQUITY: {subject.symbol}"
            )
    ...
    if len(subjects) != 12:
        raise StudyError(f"Study 03 requires exactly twelve subjects; found {len(subjects)}.")
    if len({subject.country_or_market for subject in subjects}) != 12:
        raise StudyError("Study 03 requires twelve distinct country_or_market values.")
...
def result_row(metadata: dict[str, Any]) -> dict[str, Any]:
    ...
    returned_currency = selected.get("currency")
    expected_currency = metadata.get("expected_currency")
    currency_match = (
        returned_currency == expected_currency
        if returned_currency not in (None, "") and expected_currency not in (None, "")
        else None
    )
    return {
        ...
        "yahoo_symbol_suffix": metadata["yahoo_symbol_suffix"],
        "expected_quote_type": metadata["expected_quote_type"],
        "expected_exchange_label": metadata["expected_exchange_label"],
...
def build_region_summary(metadata_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    grouped: dict[str, list[dict[str, Any]]] = {}
    for row in metadata_rows:
        grouped.setdefault(str(row["geographic_region"]), []).append(row)

    summary: list[dict[str, Any]] = []
    for region in sorted(grouped):
        rows = grouped[region]
        currencies = sorted({
            str((row.get("selected_quote_fields") or {}).get("currency"))
            for row in rows
            if (row.get("selected_quote_fields") or {}).get("currency") not in (None, "")
        })
        exchange_labels = sorted({
            str((row.get("selected_quote_fields") or {}).get("fullExchangeName"))
            for row in rows
            if (row.get("selected_quote_fields") or {}).get("fullExchangeName") not in (None, "")
        })
        summary.append(
            {
                ...
                "currency_match_count": sum(
                    (row.get("selected_quote_fields") or {}).get("currency") == row.get("expected_currency")
                    for row in rows
                ),
                "countries_or_markets_json": canonical_json(sorted(row["country_or_market"] for row in rows)),
                "currencies_json": canonical_json(currencies),
                "exchange_labels_json": canonical_json(exchange_labels),
...
def build_currency_summary(metadata_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    grouped: dict[str, list[dict[str, Any]]] = {}
    for row in metadata_rows:
        grouped.setdefault(str(row["expected_currency"]), []).append(row)

    summary: list[dict[str, Any]] = []
    for expected_currency in sorted(grouped):
        rows = grouped[expected_currency]
        returned = sorted({
            str((row.get("selected_quote_fields") or {}).get("currency"))
            for row in rows
            if (row.get("selected_quote_fields") or {}).get("currency") not in (None, "")
        ...
        summary.append(
            {
                "expected_currency": expected_currency,
                "subject_count": len(rows),
                "returned_currency_match_count": sum(
                    (row.get("selected_quote_fields") or {}).get("currency") == expected_currency
                    for row in rows
                ),
                "returned_currencies_json": canonical_json(returned),
                "symbols_json": canonical_json([row["requested_symbol"] for row in rows]),
            }
        )
    return summary
...
def run_study(
    *,
    ...
    run_id = f"{filename_utc(run_started)}_study-03-international-exchange-region"
    ...
    for index, planned in enumerate(plan):
        print(
            ...
            f"{planned.subject.symbol} / {planned.subject.country_or_market} ... ",
            end="",
            flush=True,
        )
    ...
    results_relative = "comparison/exchange-region-results.csv"
    region_summary_relative = "comparison/region-summary.csv"
    currency_summary_relative = "comparison/currency-summary.csv"
    write_csv(run_dir / results_relative, RESULT_COLUMNS, map(result_row, metadata_rows))
    write_csv(
        run_dir / region_summary_relative,
        REGION_SUMMARY_COLUMNS,
        build_region_summary(metadata_rows),
    )
    write_csv(
        run_dir / currency_summary_relative,
        CURRENCY_SUMMARY_COLUMNS,
        build_currency_summary(metadata_rows),
    )
    ...
    manifest = {
        ...
        "comparison_files": {
            "exchange_region_results": results_relative,
            "region_summary": region_summary_relative,
            "currency_summary": currency_summary_relative,
        },
        ...
        "summary": {
            ...
            "country_or_market_count": len({subject.country_or_market for subject in subjects}),
            "geographic_region_count": len({subject.geographic_region for subject in subjects}),
            "expected_currency_count": len({subject.expected_currency for subject in subjects}),
...
def print_dry_run(definition_path: Path, *, now: datetime | None = None) -> None:
    ...
    definition, endpoint, subjects = load_study_definition(definition_path)
    run_id = f"{filename_utc(run_time)}_study-03-international-exchange-region"
    plan = build_execution_plan(definition, endpoint, subjects, run_id=run_id)

    print("Study 03 international exchange and region Quote dry run")
    ...
    for planned in plan:
        ...
        print(f"  expected currency: {planned.subject.expected_currency}")
        print(f"  exchange panel label: {planned.subject.expected_exchange_label}")
        print(f"  parameter fingerprint: {planned.request_parameters_sha256}")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the twelve-subject Study 03 international exchange and region Quote baseline."
    )
    parser.add_argument(
        "--definition",
        type=Path,
        default=DEFAULT_CONFIG,
        help="Study definition JSON. Default: config/studies/study-03-international-exchange-region.json",
    )
    parser.add_argument(
        "--output-parent",
        type=Path,
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("Exchange-region results: comparison\\exchange-region-results.csv")
    print("Region summary: comparison\\region-summary.csv")
    print("Currency summary: comparison\\currency-summary.csv")
