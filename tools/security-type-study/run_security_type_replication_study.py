#!/usr/bin/env python3
r"""Run Study 02B: replicate the Quote security-type baseline with paired subjects.

The study repeats each reviewed project security type with a baseline and an independent
replication subject. Every request uses the same Quote request pattern and a prepared
anonymous cookie-plus-crumb session.
...
DEFAULT_CONFIG = REPOSITORY_ROOT / "config" / "studies" / "study-02b-security-type-replication.json"
...
PAIR_SUMMARY_COLUMNS = [
    ...
    "replication_symbol",
    "both_http_responses",
    "both_expected_symbols_returned",
    "baseline_returned_quote_type",
    "replication_returned_quote_type",
    "returned_quote_types_agree",
    "both_quote_types_match_expected",
    "baseline_field_count",
    "replication_field_count",
    "field_count_delta_replication_minus_baseline",
    "shared_top_level_field_count",
    "baseline_only_top_level_field_count",
    "replication_only_top_level_field_count",
    "top_level_field_jaccard",
    "baseline_market_state",
    "replication_market_state",
    "baseline_only_fields_json",
    "replication_only_fields_json",
]
...
def load_study_definition(path: Path) -> tuple[dict[str, Any], EndpointDefinition, list[SubjectDefinition]]:
    ...
    if endpoint.endpoint_id != "quote":
        raise StudyError("Study 02B requires endpoint_id=quote.")
    if endpoint.method != "GET":
        raise StudyError("Study 02B currently supports GET only.")
    if "{symbol}" not in canonical_json(endpoint.params):
        raise StudyError("Study 02B endpoint params must contain a {symbol} placeholder.")
    ...
    for raw in raw_subjects:
        subject = SubjectDefinition(
            ...
            representative_role=str(raw["representative_role"]),
    ...
    if len(subjects) != 24:
        raise StudyError(f"Study 02B requires exactly twenty-four subjects; found {len(subjects)}.")

    grouped: dict[str, list[SubjectDefinition]] = {}
    for subject in subjects:
        grouped.setdefault(subject.pair_id, []).append(subject)
    if len(grouped) != 12:
        raise StudyError(f"Study 02B requires exactly twelve pair_id values; found {len(grouped)}.")
    for pair_id, rows in grouped.items():
        if len(rows) != 2:
            raise StudyError(f"Pair {pair_id} must contain exactly two subjects.")
        if {row.representative_role for row in rows} != {"baseline", "replication"}:
            raise StudyError(f"Pair {pair_id} must contain baseline and replication roles.")
        if len({row.project_security_type for row in rows}) != 1:
            raise StudyError(f"Pair {pair_id} must use one project security type.")
        if len({row.expected_quote_type for row in rows}) != 1:
            raise StudyError(f"Pair {pair_id} must use one expected quote type.")
...
def capture_one(
    ...
    *,
    ...
    metadata = {
        "study_id": "study-02b-security-type-replication",
        "study_version": "0.1.0",
        "study_variable": "project_security_type_replicate",
        ...
        "returned_top_level_fields": sorted(returned_record) if returned_record else [],
...
def result_row(metadata: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "representative_role": metadata["representative_role"],
...
def build_pair_summary(metadata_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    grouped: dict[str, dict[str, dict[str, Any]]] = {}
    for row in metadata_rows:
        grouped.setdefault(str(row["pair_id"]), {})[str(row["representative_role"])] = row

    summary: list[dict[str, Any]] = []
    for pair_id in sorted(grouped):
        roles = grouped[pair_id]
        baseline = roles.get("baseline")
        replication = roles.get("replication")
        if baseline is None or replication is None:
            raise StudyError(f"Incomplete metadata pair: {pair_id}")
        baseline_fields = set(baseline.get("returned_top_level_fields") or [])
        replication_fields = set(replication.get("returned_top_level_fields") or [])
        union = baseline_fields | replication_fields
        intersection = baseline_fields & replication_fields
        baseline_selected = baseline.get("selected_quote_fields") or {}
        replication_selected = replication.get("selected_quote_fields") or {}
        baseline_type = baseline.get("returned_quote_type")
        replication_type = replication.get("returned_quote_type")
        summary.append({
            "pair_id": pair_id,
            "project_security_type": baseline["project_security_type"],
            "expected_quote_type": baseline["expected_quote_type"],
            "baseline_symbol": baseline["requested_symbol"],
            "replication_symbol": replication["requested_symbol"],
            "both_http_responses": baseline.get("http_status") is not None and replication.get("http_status") is not None,
            "both_expected_symbols_returned": bool(baseline.get("requested_symbol_returned")) and bool(replication.get("requested_symbol_returned")),
            "baseline_returned_quote_type": baseline_type,
            "replication_returned_quote_type": replication_type,
            "returned_quote_types_agree": baseline_type not in (None, "") and baseline_type == replication_type,
            "both_quote_types_match_expected": baseline.get("quote_type_match") is True and replication.get("quote_type_match") is True,
            "baseline_field_count": baseline.get("returned_field_count", 0),
            "replication_field_count": replication.get("returned_field_count", 0),
            "field_count_delta_replication_minus_baseline": int(replication.get("returned_field_count", 0)) - int(baseline.get("returned_field_count", 0)),
            "shared_top_level_field_count": len(intersection),
            "baseline_only_top_level_field_count": len(baseline_fields - replication_fields),
            "replication_only_top_level_field_count": len(replication_fields - baseline_fields),
            "top_level_field_jaccard": round(len(intersection) / len(union), 6) if union else "",
            "baseline_market_state": baseline_selected.get("marketState"),
            "replication_market_state": replication_selected.get("marketState"),
            "baseline_only_fields_json": canonical_json(sorted(baseline_fields - replication_fields)),
            "replication_only_fields_json": canonical_json(sorted(replication_fields - baseline_fields)),
        })
    return summary
...
def run_study(
    *,
    ...
    run_id = f"{filename_utc(run_started)}_study-02b-security-type-replication"
    ...
    pair_summary_relative = "comparison/security-type-pair-summary.csv"
    ...
    pair_summary_rows = build_pair_summary(metadata_rows)
    write_csv(run_dir / pair_summary_relative, PAIR_SUMMARY_COLUMNS, pair_summary_rows)
    ...
    manifest = {
        ...
        "comparison_files": {
            ...
            "security_type_pair_summary": pair_summary_relative,
        },
        ...
        "summary": {
            ...
            "complete_pair_count": sum(bool(row["both_expected_symbols_returned"]) for row in pair_summary_rows),
            "pair_returned_quote_type_agreement_count": sum(bool(row["returned_quote_types_agree"]) for row in pair_summary_rows),
            "pair_both_expected_quote_type_match_count": sum(bool(row["both_quote_types_match_expected"]) for row in pair_summary_rows),
...
def print_dry_run(definition_path: Path, *, now: datetime | None = None) -> None:
    ...
    definition, endpoint, subjects = load_study_definition(definition_path)
    run_id = f"{filename_utc(run_time)}_study-02b-security-type-replication"
    plan = build_execution_plan(definition, endpoint, subjects, run_id=run_id)

    print("Study 02B security-type replication dry run")
    ...
    for planned in plan:
        ...
        print(
            f"[{planned.sequence:02d}/{len(plan)}] "
            f"{planned.subject.symbol} / {planned.subject.project_security_type} "
            f"({planned.subject.representative_role}; pair={planned.subject.pair_id})"
        )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the twenty-four-subject Study 02B Quote security-type replication."
    )
    parser.add_argument(
        "--definition",
        type=Path,
        default=DEFAULT_CONFIG,
        help="Study definition JSON. Default: config/studies/study-02b-security-type-replication.json",
    )
    parser.add_argument(
        "--output-parent",
        type=Path,
