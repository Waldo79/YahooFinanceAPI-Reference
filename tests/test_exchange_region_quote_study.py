...
MODULE_PATH = (
    ...
    / "tools"
    / "exchange-region-study"
    / "run_exchange_region_quote_study.py"
)
SPEC = importlib.util.spec_from_file_location("run_exchange_region_quote_study", MODULE_PATH)
...
CONFIG_PATH = (
    ...
    / "config"
    / "studies"
    / "study-03-international-exchange-region.json"
)
...
def response_factory(url: str):
    ...
    subject = subjects_by_symbol()[symbol]
    return FakeResponse(
        payload(
            symbol,
            subject.expected_quote_type,
            subject.expected_currency,
            subject.expected_exchange_label,
        ),
        url=url,
    )
...
def test_definition_has_twelve_unique_international_equities():
    definition, endpoint, subjects = study.load_study_definition(CONFIG_PATH)

    assert definition["study_id"] == "study-03-international-exchange-region"
    assert endpoint.endpoint_id == "quote"
    assert len(subjects) == 12
    assert len({subject.symbol for subject in subjects}) == 12
    assert len({subject.country_or_market for subject in subjects}) == 12
    assert {subject.expected_quote_type for subject in subjects} == {"EQUITY"}
...
def test_plan_preserves_suffixes_numeric_symbols_and_order():
    definition, endpoint, subjects = study.load_study_definition(CONFIG_PATH)
    plan = study.build_execution_plan(definition, endpoint, subjects, run_id="test-run")

    assert len(plan) == 12
    assert plan[0].subject.symbol == "AAPL"
    assert plan[-1].subject.symbol == "RELIANCE.NS"
    by_symbol = {item.subject.symbol: item for item in plan}
    assert by_symbol["0700.HK"].request_parameters["symbols"] == "0700.HK"
    assert by_symbol["7203.T"].request_parameters["symbols"] == "7203.T"
    assert by_symbol["INGA.AS"].request_parameters["symbols"] == "INGA.AS"
    assert by_symbol["VALE3.SA"].request_parameters["symbols"] == "VALE3.SA"


def test_canonical_hash_ignores_object_key_order():
    first = json.loads(payload("AAPL", "EQUITY", "USD", "Nasdaq", reverse_keys=False))
    second = json.loads(payload("AAPL", "EQUITY", "USD", "Nasdaq", reverse_keys=True))

    assert json.dumps(first, separators=(",", ":")) != json.dumps(second, separators=(",", ":"))
    assert study.sha256_json(first) == study.sha256_json(second)


def test_capture_extracts_exchange_currency_and_redacts_crumb():
    definition, endpoint, subjects = study.load_study_definition(CONFIG_PATH)
    ...
    assert metadata["selected_quote_fields"]["currency"] == "USD"
    assert metadata["country_or_market"] == "United States"
...
def test_missing_requested_symbol_is_preserved_as_evidence():
    ...
    def wrong_symbol_factory(url: str):
        return FakeResponse(payload("MSFT", "EQUITY", "USD", "Nasdaq"), url=url)
...
def test_401_refreshes_once_and_never_persists_secret():
    ...
    def factory(url: str):
        ...
        return FakeResponse(payload("AAPL", "EQUITY", "USD", "Nasdaq"), url=url)
...
def test_full_synthetic_run_writes_twelve_records_and_summaries(tmp_path: Path):
    def session_factory(timeout: float):
        del timeout
        return FakeSession(response_factory)
    ...
    assert manifest["summary"]["currency_match_count"] == 12
    assert manifest["summary"]["country_or_market_count"] == 12
    ...
    assert manifest["study_definition_source_file"] == "config/studies/study-03-international-exchange-region.json"
    ...
    result_rows = read_csv(run_dir / "comparison" / "exchange-region-results.csv")
    region_rows = read_csv(run_dir / "comparison" / "region-summary.csv")
    currency_rows = read_csv(run_dir / "comparison" / "currency-summary.csv")
    assert len(result_rows) == 12
    assert len(region_rows) == 5
    assert len(currency_rows) == 10
    ...
    assert all(row["currency_match"] == "True" for row in result_rows)
...
def test_currency_mismatch_is_recorded_in_result_row():
    definition, endpoint, subjects = study.load_study_definition(CONFIG_PATH)
    planned = study.build_execution_plan(definition, endpoint, subjects, run_id="test")[1]

    def mismatch_factory(url: str):
        return FakeResponse(payload("RY.TO", "EQUITY", "USD", "Toronto"), url=url)

    _, metadata = study.capture_one(
        planned,
        FakeSession(mismatch_factory),
        maximum_attempts=1,
        now=IncrementingNow(),
        clock=IncrementingClock(),
    )
    metadata.update({"raw_response_file": "raw/x.json", "metadata_file": "metadata/x.json"})
    row = study.result_row(metadata)

    assert row["expected_currency"] == "CAD"
    assert row["currency"] == "USD"
    assert row["currency_match"] is False


def test_quote_type_mismatch_is_recorded_without_discarding_response():
    definition, endpoint, subjects = study.load_study_definition(CONFIG_PATH)
    planned = study.build_execution_plan(definition, endpoint, subjects, run_id="test")[3]

    def mismatch_factory(url: str):
        return FakeResponse(payload("INGA.AS", "ETF", "EUR", "Amsterdam"), url=url)
    ...
    assert metadata["result_classification"] == "EXPECTED_SYMBOL_RETURNED"
    assert metadata["returned_quote_type"] == "ETF"
    assert metadata["expected_quote_type"] == "EQUITY"
    assert metadata["quote_type_match"] is False


def test_dry_run_lists_all_twelve_subjects_and_redacts_crumb(capsys):
    study.print_dry_run(
        CONFIG_PATH,
        now=datetime(2026, 7, 29, 0, 0, 0, tzinfo=timezone.utc),
    )
    output = capsys.readouterr().out

    assert "Study 03 international exchange and region Quote dry run" in output
    assert "Planned requests: 12" in output
    assert "AAPL / United States" in output
    assert "INGA.AS / Netherlands" in output
    assert "RELIANCE.NS / India" in output
